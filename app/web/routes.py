...
def register_routes(app: FastAPI, config: Config, manager: JobManager, *, offline: bool = False) -> None:
    ...
    def create_review(
        ...
        video_path = _store_upload(video, Path(config.paths.videos_dir), MEDIA_SUFFIXES, "vídeo")
        ...
        for upload in references:
            if upload.filename:
                stored.append(
                    str(
                        _store_upload(
                            upload, Path(config.paths.documents_dir), SUPPORTED_SUFFIXES, "referência"
                        )
                    )
                )
        ...
        if not stored:
            raise HTTPException(
                status_code=400,
                detail="É necessário pelo menos um material de referência (arquivo ou URL).",
            )
    ...
    def get_review(job_id: str):
        ...
        return _job_or_404(manager, job_id).to_dict()
    ...
    def get_report(job_id: str):
        ...
        if not job.report_path or not Path(job.report_path).exists():
            raise HTTPException(status_code=409, detail="O relatório ainda não está pronto.")
...
def _store_upload(upload: UploadFile, directory: Path, allowed: set, label: str) -> Path:
    """Save an upload under ``directory``, rejecting unsupported extensions."""
    filename = Path(upload.filename or "").name
    if not filename:
        raise HTTPException(status_code=400, detail=f"Arquivo de {label} sem nome.")
    suffix = Path(filename).suffix.lower()
    if suffix not in allowed:
        raise HTTPException(
            status_code=400,
            detail=(
                f"Formato não suportado para {label}: '{suffix or filename}'. "
